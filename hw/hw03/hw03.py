...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(tot, i , step):
        if i >= n:
            return tot
        if i%8 == 0 or num_eights(i):
            return helper(tot+step*-1, i+1, step*-1)
        else:
            return helper(tot+step, i+1, step)
    return helper(1, 1, 1)
...
def count_coins(change):
    ...
    def counter(nom, coin):
        if nom == 0:
            ...
            return counter(nom-coin,coin) + counter(nom, next_smaller_coin(coin))
