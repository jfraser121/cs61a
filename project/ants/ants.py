    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self) == True:
                ...
                # place.ant = place.ant
            elif self.can_contain(place.ant) == True:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        while not tile.is_hive :
            if tile.bees and (lower_bound <= distance <= upper_bound):
                return random_bee(tile.bees)
            else:
                tile = tile.entrance
                distance += 1
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
            ...
            reflect = amount
        # now we do damage to all the bees on the same tile 
        for bee in list(self.place.bees):
            bee.reduce_health(reflect)
        # now take damage (other stuff like death is handled by reduce_health func in ant class. Inheritance!)
...
class BodyguardAnt(ContainerAnt):
    ...
    #     super().__init__(*args, **kwargs)
    #     self.health = 2
                                    #when to use the whole args kwargs business above?? idk man
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        """damage bees in self.place, then do any contained ant's action"""
        for bee in list(self.place.bees):
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if not gamestate.queeny:
            super().construct(gamestate)
            gamestate.queeny = True
        else:
            return None
        # END Problem 12
    ...
    def action(self, gamestate):
        ...
        tile = self.place 
        def checkbehind(tile):
            if not tile.exit:
                tile.exit.ant.double()
                checkbehind(tile.exit)
        checkbehind(tile)
        # END Problem 12
