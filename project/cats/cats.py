...
def pick(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    eligible = [paragraphs[i] for i in range(len(paragraphs)) if select(paragraphs[i])]
    if k > len(eligible)-1:
        return ""
    else:
        return eligible[k]
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    # else:
    #     best_word = min([diff_function(typed_word, word, limit) for word in word_list])
    # fric
    else:
...
def time_per_word(words, times_per_player):
    ...
    for player_times in times_per_player:
        player = []
        for i, time in enumerate(player_times):
            if i > 0:
                player.append(time-player_times[i-1])
        times.append(player)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
        ...
        ### didn't realise the above were already defined lol
    # racers = []
    # for _ in get_all_times(match):
    #     racers.append([])
    # for w, word in enumerate(get_all_words(match)):
    #     quickest_time = time(match, 0, w)
    #     quickest_player = 0
    #     for i, player in enumerate(get_all_times(match)):
    #         if time(match, i, w) < quickest_time:
    #             quickest_time = time(match, i, w)
    #             quickest_player = i
    #     racers[quickest_player].append(word)
    # return racers

    racers = [[] for player in player_indices]
    for w in word_indices:
        quickest_time = time(match, 0, w)
        quickest_player = 0
        for p in player_indices:
            p_time = time(match, p, w)
            if p_time < quickest_time:
                quickest_time = p_time
                quickest_player = p
        racers[quickest_player].append(get_word(match, w))
    return racers
