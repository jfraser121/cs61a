...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i <= num_rolls:
        ...
        if roll == 1:
            ...
            sum_rolls += roll
            # print(f"DEBUG: roll was {roll}, sum_rolls is {sum_rolls}")
        i += 1
    if sow_sad:
        return 1
    else:
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 = update(strat0,score0,score1,dice)
        elif who == 1:
            strat1 = strategy1(score1,score0)
            score1 = update(strat1,score1,score0,dice)
        who = 1 - who 
    # END PROBLEM 5
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score<100:
        while opp_score<100:
            if base != strategy(score,opp_score):
                return False
            opp_score+=1
        opp_score=0
        score+=1
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    i, best_score, best_roll = 1, 0, 1
    while i < 11:
        score = make_averaged(roll_dice, total_samples)(i, dice)
        print(f"DEBUG: score for {i} rolls is {score}, current best score is {best_score} with {best_roll} rolls")
        if score > best_score:
            best_roll = i
            best_score = score
        i+=1
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # else:
    #     return num_rolls
    if simple_update(0,score,opponent_score) - score >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    # tail = tail_points(opponent_score)
    # print(f"DEBUG: tail_score: {tail}")
    # score_post_tail = (score + tail)
    # print(f"DEBUG: score:{score}, score_post_tail:{score_post_tail}")
    # print(f"DEBUG: square_update:{square_update(0,score_post_tail, opponent_score)}")
    # if tail >= threshold or square_update(0,score, opponent_score)-score >= threshold:
    #     return 0
    # else:
    #     return num_rolls    
